...
// Establish when one variable starts and another ends to make jobs easier.
size_t x_start = 0;
...
class FG_eval {
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += 18000*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set the upper and lower limits of delta to -20 and 20 degrees (values in radians).
  ...
  // Return the first actuator values and predicted x,y values. The variables can be accessed with `solution.x[i]`.
